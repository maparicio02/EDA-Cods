...
def main(image_path, threshold, min_size):
    ...
    original_size = image_np.size
    quadtree_size = count_nodes(root)

    fig, axes = plt.subplots(1, 2, figsize=(12, 6))
    axes[0].imshow(image_np, cmap='gray')
    axes[0].set_title(f'Imagen Original\nTamaño: {original_size} bytes')
    axes[0].axis('off')

    axes[1].imshow(reconstructed_image, cmap='gray')
    axes[1].set_title(f'Imagen Reconstruida\nTamaño (Quadtree): {quadtree_size} nodos')
    axes[1].axis('off')
